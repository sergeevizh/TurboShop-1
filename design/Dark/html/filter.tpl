    ...
    <form method="get" action="{url page=null}">
		...
		<article class="card-group-item">
            ...
            <div class="filter-content collapse {if $is_mobile === false && $is_tablet === false}show{/if}" id="collapse{$f->id}">
                <div data-toggle="buttons" class="card-body">
                    {foreach $f->options as $k=>$o}
                    <label style="background-color:{$o->value|escape}" class="btn btn-sm {if $o->value == '#ffffff'}white{/if} {if $filter_features.{$f->id} && in_array($o->value,$filter_features.{$f->id})}active{/if}">
