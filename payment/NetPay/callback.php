...
$query = $turbo->db->placehold("SELECT * FROM __payment_methods WHERE module=? LIMIT 1", 'NetPay');
$turbo->db->query($query);
$payment_method = $turbo->db->result();
...
if (($newarr[2][1] == 'APPROVED') &
    ...
    $order = $turbo->orders->get_order(intval($newarr[0][1]));
    if(empty($order))
