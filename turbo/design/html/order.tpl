    ...
    <div class="row">
        <div class="col-lg-12 col-md-12">
            <div class="wrap_heading">
                ...
                <div data-hint="Распечатать" class="boxes_inline hint-bottom-middle-t-info-s-small-mobile  hint-anim ml-h">
                    <a href="{url view=print id=$order->id}" target="_blank" title="Распечатать" class="print_block">
                ...
                <div class="box_btn_heading ml-h hidden-xs-down">
                    <div class="add_order_marker">
                        <span class="fn_ajax_label_wrapper">
                            <span class="fn_labels_show box_labels_show box_btn_heading ml-h">{include file='svg_icon.tpl' svgId='tag'} <span>Выберите метку</span> </span>
                            <div class='fn_labels_hide box_labels_hide'>
                                <span class="heading_label">Метки <i class="fn_delete_labels_hide btn_close delete_labels_hide">{include file='svg_icon.tpl' svgId='delete'}</i></span>
                    ...
                    <div class="">
                        <div id="fn_purchase" class="turbo_list turbo_list_order">
                            <div class="turbo_list_head">
                                ...
                                <div class="turbo_list_heading turbo_list_order_name">Название/вариант</div>
                                <div class="turbo_list_heading turbo_list_price">Цена {$currency->sign|escape}</div>
                                <div class="turbo_list_heading turbo_list_count">Кол-во, {$settings->units|escape}
								</div>
                                <div class="turbo_list_heading turbo_list_order_amount_price">Сумма продаж</div>
							</div>
									...
									<div class="turbo_list_row">
										...
										<div class="turbo_list_boding turbo_list_order_name">
											<div class="boxes_inline">
												...
												{if !$order->closed}
												{if !$purchase->product}
												<span data-hint="Товар был удалён" class="text_warning hint-bottom-middle-t-info-s-small-mobile  hint-anim"><i class="icon-info icons font-xs"></i></span>
												{elseif !$purchase->variant}
												<span data-hint="Вариант товара был удалён" class="text_warning hint-bottom-middle-t-info-s-small-mobile  hint-anim"><i class="icon-info icons font-xs"></i></span>
												{elseif $purchase->variant->stock < $purchase->amount}
													<span data-hint="На складе остал{$purchase->variant->stock|plural:'ся':'ось'} {$purchase->variant->stock} товар{$purchase->variant->stock|plural:'':'ов':'а'}" class="text_warning hint-bottom-middle-t-info-s-small-mobile  hint-anim"><i class="icon-info icons font-xs"></i></span>
													{/if}
													{/if}
													{if $purchase->variant_name}
													<span class="text_grey">Вариант: {$purchase->variant_name|escape}</span>
													{/if}
													{if $purchase->sku}
													<span class="text_grey">Артикул {$purchase->sku|default:"&mdash;"}</span>
													{/if}
													{else}
													<div class="text_grey">{$purchase->product_name|escape}</div>
													{if $purchase->variant_name}
													<div class="text_grey">Вариант: {$purchase->variant_name|escape}</div>
													{/if}
													{if $purchase->sku}
													<div class="text_grey">Артикул{$purchase->sku|default:"&mdash;"}</div>
													{/if}
                                                    {/if}
							...
							<div class="row mt-1">
								<div class="col-lg-6 col-md-12">
									<div class="autocomplete_arrow">
										<input type="text" name="new_purchase" id="fn_add_purchase" class="form-control" placeholder="Выберите товар чтобы добавить его">
									</div>
								</div>
								<div class="col-lg-6 col-md-12">
									{if $purchases}
                                    <div class="text_dark text_500 text-xs-right mr-1 mt-h">
                                        <div class="h5">Всего: {$subtotal} {$currency->sign|escape}</div>
									</div>
									{/if}
								</div>
							</div>
							...
							<div class="">
								<div class="turbo_list turbo_list_order">
									<div class="turbo_list_body">
										<div class="turbo_list_body_item">
											<div class="turbo_list_row  d_flex">
												<div class="turbo_list_boding turbo_list_ordfig_name">
													<div class="text_600 text_dark">Скидка</div>
												</div>
										...
										<div class="turbo_list_body_item">
											<div class="turbo_list_row d_flex">
												<div class="turbo_list_boding turbo_list_ordfig_name">
													<div class="text_600 text_dark">Купон{if $order->coupon_code} ({$order->coupon_code}){/if}</div>
												</div>
												<div class="turbo_list_boding turbo_list_ordfig_val">
													<div class="input-group">
										...
										<div class="turbo_list_body_item">
											<div class="turbo_list_row  d_flex">
												<div class="turbo_list_boding turbo_list_ordfig_name">
													<div class="text_600 text_dark boxes_inline">Доставка</div>
													<div class="boxes_inline">
								...
								<div class="row ">
									<div class="col-lg-4 col-md-12 mt-2">
										<div class="heading_label">Выбранный способ оплаты</div>
										<div class="">
									...
									<div class="col-lg-8 col-md-12">
										<div class="text_dark text_500 text-xs-right mr-1 mt-1">
											<div class="h5">Итого: {$order->total_price} {$currency->sign|escape}</div>
										</div>
										<div class="text_grey text_500 text-xs-right mr-1 mt-1">
											{if $payment_method}
                                            <div class=" text-xs-right">
                                                <div class="h5">К оплате: {$order->total_price|convert:$payment_currency->id} {$payment_currency->sign}</div>
											</div>
											{/if}
										</div>
									</div>
								...
								<div class="row">
									<div class="col-lg-12 col-md-12 mt-1">
										<label class="switch_label boxes_inline">Заказ оплачен</label>
				...
				<div class="boxed fn_toggle_wrap min_height_230px">
					...
					<div class="toggle_body_wrap on fn_card">
						<div class="box_border_buyer">
							<div class="mb-1">
								<div class="heading_label boxes_inline">Дата заказа:</div>
							...
							<div class="mb-1">
								<div class="heading_label">Телефон</div>
							...
							<div class="mb-1">
								<div class="heading_label">Адрес <a href="https://www.google.com/maps/search/{$order->address|escape}?hl=ru" target="_blank"><i class="fa fa-map-marker"></i> на карте</a></div>
								<textarea name="address" class="form-control short_textarea">{$order->address|escape}</textarea>
							</div>
							<div class="mb-1">
								<div class="heading_label">Комментарий</div>
								<textarea name="comment" class="form-control short_textarea">{$order->comment|escape}</textarea>
							</div>
							<div class="mb-1">
								<div class="heading_label boxes_inline">IP покупателя: <a href="https://who.is/whois-ip/ip-address/{$order->ip}" target="_blank"><i class="fa fa-map-marker"></i> whois</a></div>
							...
							<div class="mb-1">
								<div style="position:relative;">
                                    ...
                                    <div style="position:relative;">
                                        ...
                                        <input type="text" class="fn_user_complite form-control" placeholder="Выберите пользователя" />
									</div>
									{else}
                                    <div class="fn_user_row">
                                        ...
                                        {else}
										<div class="text_grey">не в группе</div>
                                        {/if}
									</div>
									{/if}
								</div>
						...
						<div class="box_border_buyer">
							<div class="">
								<div class="form-group">
									<div class="heading_label">Примечание администратора к заказу</div>
		...
		<div class="row">
			<div class="col-lg-12 col-md-12 mb-2">
				...
				<div class="checkbox_email float-sm-right text_dark mr-1">
					...
					<span>Отправить e-mail о состоянии заказа</span>
				</div>
			</div>
		</div>
	</form>
