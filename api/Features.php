...
{	
	...
	public function get_options($filter = array())
	{
		...
		if(isset($filter['features']))
			...
			{
				$features_filter .= $this->db->placehold('AND (po.feature_id=? OR po.product_id in (SELECT product_id FROM t_options WHERE feature_id=? AND value in (?@) )) ', $feature, $feature, $value);
			}
